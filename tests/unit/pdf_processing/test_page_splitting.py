...
logging.basicConfig(
    level=logging.INFO, format=log_fmt, filename=log_file, filemode="a", force=True
)
...
def test_validate_page_splitting(json_dir, data_dir):
    ...
    for result in results:
        ...
        assert (
            "filename_match_found" in result
        ), f"{result['json_file']}: missing 'filename_match_found'"
        assert (
            "page_count_matches" in result
        ), f"{result['json_file']}: missing 'page_count_matches'"
...
def test_validate_page_splitting_page_count_mismatch(tmp_path):
    ...
    json_data = {
        ...
        "content": [{"page_number": 1}, {"page_number": 2}, {"page_number": 3}],
    }
    ...
    assert len(results) == 1, "Should process the file"
    assert results[0]["filename_match_found"], "Filename should match"
    assert (
        results[0]["last_page_number_from_json_content"] == 3
    ), "Should have 3 pages in JSON"
    assert results[0]["expected_page_count_from_pdf"] == 5, "Should expect 5 pages"
    assert not results[0]["page_count_matches"], "Page counts should NOT match"
...
def test_validate_page_splitting_correct_match(tmp_path):
    ...
    assert len(results) == 1, "Should process the file"
    assert results[0]["filename_match_found"], "Should find a match"
    assert (
        results[0]["matched_expected_filename"] == "test-document.pdf"
    ...
    assert results[0]["page_count_matches"], "Page counts SHOULD match"
